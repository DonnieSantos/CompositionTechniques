...
namespace ChessComposition
{
    ...
    public class Piece
    {
        ...
        public List<IRule> LegalRules { get; set; }
        public List<IRule> IllegalRules { get; set; }
        ...
        public Piece(int x, int y, List<IRule> legalRules, List<IRule> illegalRules)
        {
            X = x;
            Y = y;
        ...
        public bool Move(int dx, int dy)
        {
            ...
            foreach (IRule rule in IllegalRules)
            {
            ...
            foreach (IRule rule in LegalRules)
            {
                if (rule.IsLegalMove(X, Y, dx, dy))
                {
                    X = dx;
